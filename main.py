"""main.py — entry point.
python main.py --headless [--maze random|simple] [--size N] [--seed N]
python main.py --2d [--maze ...] [--size N] [--seed N] [--no-menu]
...
def build_sim(maze="random", seed=None, size=16):
    ...
    if maze == "simple":
        ...
        return Simulator(build_simple_maze())
    return Simulator(generate_random_maze(seed, size=size))


def run_headless(maze, seed, size):
    sim = build_sim(maze, seed, size)
    while sim.phase not in ("OPTIMIZE", "DONE"):
        sim.step()
    print(f"maze       : {maze} {size}x{size} seed={seed}")
    ...
    print("OK: flood-fill reached goal + returned; A* shorter or equal.")
...
def main():
    ...
    args = ap.parse_args()
    if args.headless:
        run_headless(args.maze, args.seed, args.size)
    elif args.two_d:
